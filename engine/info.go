...
type TorrentLog struct {
	...
	StoragePath string
	Files       []TorrentLogFile
}

type TorrentLogFile struct {
	Path     string
	Priority int
}
...
var indexMapping = map[int]interface{}{
	0: torrent.PiecePriorityNone,
	1: torrent.PiecePriorityNormal,
	2: torrent.PiecePriorityHigh,
}

func (i *TorrentLogFile) getPriority() interface{} {
	return indexMapping[i.Priority]
}

func  ApplyPriority(file *torrent.File,level int) {
	switch level {
	case 0:
...
func (i *TorrentLogFile) setPriority(priority int) {
	i.Priority = priority
}

func GetPriorityIndex(idx interface{}) int {
	for mappingIdx, elm := range indexMapping {
		if elm == idx {
			return mappingIdx
		}
	}
